...
MEM_DIRS = [
    ...
    "/cifs/data/tserre/CLPS_Serre_Lab/connectomics/mag1_membranes",
    "/gpfs/data/tserre/data/tmp_ding_segmentations/connectomics/mag1_membranes",
    "/gpfs/data/tserre/data/tmp_ding_segmentations/mag1_membranes",
]
...
def execute_ding_load(sel_coor, idx, config, dtype=np.uint32, shape=(128, 128, 128), dc_path="/cifs/data/tserre/CLPS_Serre_Lab/connectomics"):
    ...
    for di in BU_DIRS:
        ...
        if os.path.exists(path):
            z = np.load(path)
            if 384 not in [x for x in z.shape]:
                continue   # There are weird debugging volumes we need to ignore
...
with Parallel(max_nbytes=None, n_jobs=-1, require='sharedmem') as parallel:  # n_jobs=32
# with Parallel(max_nbytes=None, n_jobs=-1) as parallel:  # n_jobs=32
    for zidx, z in pbar:  # tqdm(enumerate(unique_z), total=len(unique_z), desc="Z-slice main clock"):
        ...
        if not skip_processing:
            ...
            z_sel_coors_main, z_sel_coors_merge = get_main_merge_splits(pre_all_coords)
            ...
            start = time.time()
            for it_merge_coors in z_sel_coors_merge:
                vols_vox = parallel(delayed(batch_load)(sel_coor, config, path_extent, dtype, remap_labels, remap_type, min_vol_size, in_place, max_vox, mins) for sel_coor in tqdm(it_merge_coors, desc='Z (loading merges): {}'.format(z)))
