        ...
        with Parallel(n_jobs=batch_size, prefer="threads") as parallel: 
            for scale in scales:
                with tf.Session() as sess:
                    ...
                    # Resize images
                    it_res_shape = (res_shape * np.asarray([1, scale, scale])).astype(int)
                    it_vol_mem = parallel(
                        delayed(
                            lambda x, y: resize(
                                x,
                                ...
                                anti_aliasing=True,
                                preserve_range=True,
                                order=resize_order))(vm, it_res_shape[::-1][:-1]) for vm in vol_mem)
                    it_vol_mem = np.asarray(it_vol_mem).astype(vol_mem.dtype)
                    bounds = np.asarray(it_vol_mem.shape)[:-1] - np.asarray(cube_size)

                    # Loop through data
                    message = "Preferring scale: {}, Loss: {}"
                    pb = tqdm(range(steps), desc=message.format(scale, "...", "..."), total=steps)
                    losses = []
                    best_loss = 10.
                    for step in pb:
                        data = parallel(delayed(sample_batch)(it_vol_mem, bounds, cube_size) for idx in range(batch_size))
                        ...
                        pb.set_description(message.format(scale, best_loss))
                    best_losses.append(np.min(losses))
                    # from matplotlib import pyplot as plt;imn=16;plt.subplot(121);plt.imshow(best_result["image_label"][0, imn]);plt.subplot(122);plt.imshow(np.argmax(best_result["logits"][0, imn], -1));plt.show()
        best_losses = np.asarray(best_losses)
        from matplotlib import pyplot as plt
        plt.plot(best_losses)
        np.savez("best_losses", best_losses=best_losses, scales=scales)
        best_scale_idx = np.argmin(best_losses)
        selected_scale = scales[best_scale_idx]
        print("Final scale: {}".format(selected_scale))
