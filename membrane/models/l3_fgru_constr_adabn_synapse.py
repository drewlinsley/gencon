...
def cell_type_experiment_params(
        train_name=None,
        test_name=None,
        train_shape=None,
        test_shape=None,
    ...
    if train_shape is None:
        train_shape = [0]
    reader_image_shape = [64, 128, 128, 2]
    reader_label_shape = [64, 128, 128, 6]
    # model_image_shape = [64, 128, 128, 6]  # muller-only, otherwise set to 7
    # model_label_shape = [64, 128, 128, 6]
    model_image_shape = [64, 128, 128, 1]  # muller-only, otherwise set to 7
    model_label_shape = [64, 128, 128, 1]
    exp = {
        'lr': [1e-3],
        'loss_function': ['bce'],
        # 'loss_function': ['cce'],
        'optimizer': ['nadam'],
        'training_routine': ['seung'],
        'train_input_shape': model_image_shape,  # [96, 96, 96, 2],
        'train_label_shape': model_label_shape,  # [96, 96, 96, 1],  # JUST RIBBONS
        'test_input_shape': model_image_shape,  # [128, 128, 128, 2],
        'test_label_shape': model_label_shape,  # [128, 128, 128, 1],
        'train_stride': [1, 1, 1],
        'test_stride': [1, 1, 1],
        'tf_dtype': tf.float32,
        'np_dtype': np.float32
...
def synapse_experiment_params(
    ...
    if train_shape is None:
        train_shape = [0]
    reader_image_shape = [64, 128, 128, 2]
    reader_label_shape = [64, 128, 128, 1]
    model_image_shape = [64, 128, 128, 1]  # muller-only, otherwise set to 7
    model_label_shape = [64, 128, 128, 1]
    exp = {
        'lr': [1e-3],
...
def build_model_no_membrane(data_tensor, reuse, training, output_channels, merge="none", scope_name=None):
    ...
    if scope_name is None:
        scope_name = 'cnn'
    im_data_tensor = data_tensor / 255.
    ...
    with tf.variable_scope('{}_readout'.format(scope_name), reuse=reuse):
...
def build_model(data_tensor, reuse, training, output_channels, merge="none", scope_name=None):
    """Create the hgru from Learning long-range..."""
    filters = [30]
    kernel_size = [1, 5, 5]
    up_kernel = [1, 2, 2]
...
def main_cell_type(
    ...
    if evaluate:
        return model_fun.evaluate_model(
            ...
            # force_return_model=force_return_model,
            version=version,
            build_model=build_model,  # _no_membrane,
            experiment_params=cell_type_experiment_params,
            checkpoint=checkpoint,
            force_meta=force_meta,
            force_return_model=force_return_model,
            full_volume=full_volume,
