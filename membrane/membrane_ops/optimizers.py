...
def get_optimizer(
    ...
    with tf.control_dependencies(update_ops):
        if optimizer == 'adam':
            ...
            optim = lambda x: tf.train.AdamOptimizer(x, epsilon=1e-3)
...
def check_and_clip_grads(
    ...
    if len(null_grads) and not skip_check:
    ...
    if clip_gradients:
        ...
        if global_step is None:
            return optim.apply_gradients(gvs, global_step)
        else:
