...
def pad_vol(vol, shp, margin=None):
    ...
    def add_pads(shp, margin):
        shp_signs = np.sign(shp)
...
class Canvas(object):
  ...
  def predict(self, pos, logit_seed, extra_fetches):
    ...
    if 0:
        ...
        plt.subplot(243)
        plt.title('Membrane')
        plt.imshow(img[4, :, :, 1], cmap='gray')
        plt.subplot(244)
        plt.title('Seed')
        plt.imshow(logit_seed[4, :, :])
        plt.colorbar()
        plt.subplot(245)
        plt.imshow(logits[:, int(logits.shape[2] / 2), :, 0])
        plt.colorbar()
        plt.subplot(246)
        plt.imshow(img[:, int(logits.shape[2] / 2), :, 0], cmap='gray')
        plt.colorbar()
        plt.subplot(247)
        plt.imshow(img[:, int(logits.shape[2] / 2), :, 1], cmap='gray')
        plt.subplot(248)
        plt.imshow(logit_seed[:, int(logits.shape[2] / 2), :])
        plt.show()

        plt.subplot(131)
        plt.imshow(logits[8, 0, :, :])
        plt.colorbar()
        plt.subplot(132)
        plt.imshow(img[8, :, :, 0], cmap='gray')
        plt.subplot(132)
        plt.imshow(img[8, :, :, 1], cmap='gray')
        plt.colorbar()
  ...
  def segment_all(self, seed_policy=seed.PolicyPeaks, debug=False):
    ...
    with timer_counter(self.counters, 'segment_all'):
      ...
      for count, pos in TimedIter(self.seed_policy, self.counters, 'seed-policy'):
        ...
        prev_id = self.seed_policy.reserved_ids[count - 1]
        ...
        if prev_id is not None:
            self.log_info('Starting segmentation at %r (zyx). Building from prev seg_id %s.' % (pos, prev_id))
        else:
            self.log_info('Starting segmentation at %r (zyx). Trying new seg_id %s.' % (pos, self._max_id))
        ...
        if prev_id is not None:
          ...
          if debug:
            original_mask = self.init_seg_volume[sel] == prev_id
            original_sum = original_mask.sum()
            overlap = mask & original_mask
            overlap_sum = overlap.sum()
            explained = (float(overlap_sum) / float(original_sum)) * 100.
            self.log_info('Created supervoxel:%d  seed(zyx):%s  size:%d  iters:%d  cover_of_original:%d%%',
                          prev_id,
                          pos,
                          actual_segmented_voxels,
                          num_iters,
                          explained)
          self.overlaps[prev_id] = np.array([overlapped_ids, counts])
          self.origins[prev_id] = storage.OriginInfo(pos, num_iters, t_seg)
        else:
          self._max_id += 1
  ...
  def init_segmentation_from_volume(self, volume, corner, end,
    ...
    self._max_id = np.max(init_seg)

    # Force an empty canvas
    init_seg = np.zeros_like(init_seg)
...
class Runner(object):
    ...
    with timer_counter(self.counters, 'volstore-open'):
      ...
      def _open_or_none(settings):
        ...
        return storage.decorated_volume(
            settings, cache_max_bytes=int(1e7), cache_compression=False)[0]
      self._mask_volumes = {}
  ...
  def make_canvas(self, corner, subvol_size, **canvas_kwargs):
    ...
    with timer_counter(subvol_counters, 'load-image'):
      ...
      src_corner, src_size = storage.clip_subvolume_to_bounds(
          src_corner, src_size, self._image_volume[..., 0])
    ...
    if self.request.HasField('init_segmentation'):
      ...
      canvas.init_seg_volume = self.init_seg_volume
      # Remove shift# voxels from the init and save those for reassignment later.
      canvas.shifts = self.shifts
      if hasattr(self, 'previous_origins'):
          canvas.previous_origins = self.previous_origins
    else:
      canvas.shifts = None
