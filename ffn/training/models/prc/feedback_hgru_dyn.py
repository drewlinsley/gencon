...
class hGRU(object):
    ...
    def resize_x_to_y(
        ...
        n, d, h, w, c = x.get_shape()
        if mode == 'resize':
            n, d, h, w, c = x.get_shape()
            x = tf.reshape(x, [n * d, h, w, c])
            yn, yd, yh, yw, yc = y.get_shape()
            y = tf.reshape(y, [yn * yd, yh, yw, yc])
            x = tf.image.resize_bilinear(
                x,
                y.get_shape(),
                align_corners=True)
            x = tf.reshape(x, [yn, yd, yh, yw, yc])
        elif mode == 'transpose':
            # y_size = tf.Tensor.get_shape(y)  # ().as_list()
    ...
    def circuit_input(self, x, h2, fb, var_scope):
        ...
        with tf.variable_scope(var_scope, reuse=True):
            ...
            horizontal_kernels = tf.get_variable("W_inh")
        ## COMPUTE g1a
        g1a_intermediate = self.generic_combine(
            x,
            fb,
            gain_a_kernels_x, gain_a_kernels_y, gain_a_kernels_xy)
        g1a_intermediate = tf.nn.conv3d(
            g1a_intermediate, gain_a_kernels_mlp,
    ...
    def circuit_output(self, h1, fb, var_scope):
        ...
        with tf.variable_scope(var_scope, reuse=True):
            ...
            horizontal_kernels = tf.get_variable("W_exc")
        g2_intermediate = self.generic_combine(
            h1,
            fb,
            mix_kernels_x, mix_kernels_y, mix_kernels_xy)
        g2_intermediate = tf.nn.conv3d(
            g2_intermediate, mix_kernels_mlp,
    ...
    def input_integration(self, x, c1, h2, var_scope):
        ...
        return self.recurrent_nl(x - self.recurrent_nl(stacked))  #  + 1 #self.recurrent_nl(stacked)
    ...
    def full(self, i0, x, l0_h2, l0_fb, l1_h2, l1_fb, l2_h2, l2_fb):
        ...
        e1=1
        if l0_h2 is None:
            l0_h2 = tf.zeros_like(x)
            l0_fb = tf.zeros_like(x)
        ...
        # HGRU 1
        if l1_h2 is None:
            l1_h2 = tf.zeros_like(ff0)
            l1_fb = tf.zeros_like(ff0)
        idx = 1
        if self.adapation:
        ...
        # HGRU 2
        if l2_h2 is None:
            l2_h2 = tf.zeros_like(ff1)
            l2_fb = tf.zeros_like(ff1)
        idx = 2
        if self.adapation:
            with tf.variable_scope('hgru_%s' % idx, reuse=True):
                eta1 = tf.get_variable("eta1")
    ...
    def build_new(self, x, seed):
        """Run the backprop version of the Circuit."""
        self.prepare_tensors()
        i0 = tf.constant(0)

        # Calculate l2 hidden state size
        l0_h2, l0_fb, l1_h2, l1_fb, l2_h2, l2_fb = None, None, None, None, None, None
        self.full(i0, x, l0_h2, l0_fb, l1_h2, l1_fb, l2_h2, l2_fb)
        return
